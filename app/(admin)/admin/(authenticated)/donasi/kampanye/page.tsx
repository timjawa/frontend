...
import KampanyeTableAction from "./KampanyeTableAction";
      ...
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-3 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-8">
        <div className="relative bg-white/80 dark:bg-gray-900/50 backdrop-blur-sm rounded-2xl ring-1 ring-slate-100 dark:ring-gray-800 shadow-sm overflow-hidden">
          <div className="px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 dark:border-gray-800">
            ...
            <div className="flex items-center justify-start sm:justify-end gap-2.5 flex-wrap">
              ...
              <div className="relative w-full sm:w-48 lg:w-56">
                ...
                <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Cari kampanye..." className="w-full pl-9 pr-4 py-2 text-sm rounded-lg bg-slate-50 dark:bg-gray-800 border border-slate-200 dark:border-gray-700 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 focus:bg-white dark:focus:bg-gray-900 outline-none transition-all text-gray-700 dark:text-gray-200" />
              </div>
              <Link href="/admin/donasi/kampanye/create" className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 active:scale-95 transition-all shadow-sm shadow-blue-200 dark:shadow-none whitespace-nowrap">
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-slate-50/80 dark:bg-gray-800/50">
                  {["No", "Judul", "Jenis", "Kecamatan", "Target", "Terkumpul", "Disalurkan", "Status", "Aksi"].map((head) => (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
                {loading ? Array.from({ length: 5 }).map((_, i) => (
                  ...
                  <tr><td colSpan={9} className="px-6 py-12 text-center text-slate-400 dark:text-gray-500"><HiOutlineHeart className="w-10 h-10 mx-auto mb-2 text-slate-300 dark:text-gray-600" /><p>Belum ada kampanye donasi.</p></td></tr>
                ) : data.map((item, index) => (
                    ...
                    <td className="px-6 py-4 text-right">
                      <KampanyeTableAction
                        ...
                        onDelete={(id) => {
                          triggerConfirm(
                            "Hapus Kampanye?",
                            "Apakah Anda yakin ingin menghapus kampanye donasi ini? Data kampanye akan terhapus secara permanen.",
                            async () => {
                              try {
                                await api.delete(`/api/admin/donasi/kampanye/${id}`);
          ...
          {confirmModal && confirmModal.isOpen && (
            <div className="absolute inset-0 z-30 flex items-center justify-center p-4 bg-white/70 dark:bg-gray-950/70 backdrop-blur-sm animate-in fade-in duration-150">

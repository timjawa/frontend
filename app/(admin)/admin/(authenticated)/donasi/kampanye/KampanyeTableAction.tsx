...
export default function KampanyeTableAction({ id, onDelete }: { id: string | number; onDelete: (id: string | number) => void }) {
  ...
  return (
    <div className="relative inline-block text-left">
      ...
      <Dropdown isOpen={isOpen} onClose={() => setIsOpen(false)} className="w-40 right-0 top-full">
        <div className="py-1">
          <DropdownItem tag="a" href={`/admin/donasi/kampanye/${id}`} className="flex items-center gap-2">
            <HiOutlineEye className="w-4 h-4" />
            Detail
          </DropdownItem>
          <DropdownItem tag="a" href={`/admin/donasi/kampanye/${id}/edit`} className="flex items-center gap-2">

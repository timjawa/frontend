...
export default function CreatePeringatanDiniPage() {
  ...
  useEffect(() => {
    const fetchKecamatan = async () => {
      try {
        const res = await fetch(`${API_BASE}/kecamatan?per_page=100`);
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!payload.berlaku_hingga) {
        // null or empty string should be handled
        payload.berlaku_hingga = ""; 
      }

      const res = await fetch(`${API_BASE}/peringatan-dini`, {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Berlaku Hingga */}
            <div className="space-y-2">
              <label htmlFor="berlaku_hingga" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                Berlaku Hingga
              </label>
              <input
                type="datetime-local"
                ...
                onChange={handleChange}
                className={inputClass("berlaku_hingga")}
              />
              <p className="text-xs text-gray-500">Kosongkan jika peringatan berlaku selamanya hingga dicabut secara manual.</p>
              {errors.berlaku_hingga && <p className="text-xs text-red-500">{errors.berlaku_hingga}</p>}
            </div>

...
export default function PeringatanDiniTableAction({ id, onDeleted }: Props) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteError(null);
    try {
      await api.delete(`/api/peringatan-dini/${id}`);
      setShowConfirm(false);
      onDeleted?.(id);
    } catch (err: any) {
      setDeleteError(err.response?.data?.message || "Gagal menghapus peringatan dini.");
    } finally {
      setDeleting(false);
    }
  };
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-gray-100 dark:border-gray-800 animate-in fade-in zoom-in-95">
            <div className="flex flex-col items-center text-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-base font-bold text-gray-900 dark:text-white">Hapus Peringatan?</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Tindakan ini tidak dapat dibatalkan. Peringatan dini akan dihapus permanen.
                </p>
              </div>
            </div>
